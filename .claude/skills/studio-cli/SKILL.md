---
name: studio-cli
description: How to drive Paper Couch Studio from the terminal with `npm run studio` - every command, its options, selectors, plan files, output, exit codes and the gotchas an agent hits
---

# The Studio command line

Everything the Paper Couch Studio UI does can be done from a terminal, on the same workspace, through

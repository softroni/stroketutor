...
import { lessonSheetSvg, sheetGrid, traceSvg, type PreviewTrace } from '../../src/studio/preview'
...
export const previewCommands: Command[] = [
  command(
    ...
    async (ctx, args) => {
      ...
      const svgText = traceSvg(trace, { labels: !args.values['no-labels'] })
      await writePicture(ctx, args, svgText, withExtension(file, '.preview'), PREVIEW_EDGE, { lines: trace.strokes.length, colours: trace.fills.length })
    },
  ),

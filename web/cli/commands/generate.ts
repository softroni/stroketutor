...
interface Candidate {
  ...
  /** Built from a plan written by hand (`--plan`). */
  planned?: boolean
}
...
function candidateLines(candidate: Candidate): string[] {
  return [
    candidate.model
      ...
      : candidate.planned
        ? 'Built from the trace and your plan.'
...
async function generateLesson(ctx: Context, args: Parsed, accepts: 'svg' | 'raster' | 'any') {
  ...
  if (withoutModel && !reference.svg) throw new CliError(`${planFile ? '--plan' : '--no-model'} needs an SVG: a photo has no lines to trace.`)
  ...
  if (args.values['dry-run']) {
    ...
    ctx.out.result(
      ...
      () => [
        ...
        withoutModel ? (rawPlan ? 'No model would be asked; the plan would be used.' : 'No model would be asked.') : `Model: ${ctx.flags.model || deps.defaultModel || 'none: pass --model'}; key ${deps.apiKey ? 'configured' : 'missing'}.`,
